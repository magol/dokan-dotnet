...
namespace DokanNet.Tests
{
    ...
    internal sealed class DokanOperationsFixture
    {
        ...
        private FileInformation[] Named(IEnumerable<FileInformation> infos)
        ...
        internal void ExpectCreateFileToFail(string path, NtStatus result, bool closeFile = false)
        {
            ...
            _operations
                ...
                .Callback((string fileName, FileAccess _access, FileShare _share, FileMode _mode, FileOptions options, FileAttributes _attributes, DokanFileInfo info)
                    => Trace($"{nameof(IDokanOperations.CreateFile)}[{(closeFile ? Interlocked.Increment(ref _pendingFiles) : Interlocked.Read(ref _pendingFiles))}] **{result}** (\"{fileName}\", [{_access}], [{_share}], {_mode}, [{options}], [{_attributes}], {info.Log()})"))
                .Verifiable();
        ...
        internal void ExpectCloseFile(string path, object context = null, bool isDirectory = false, bool deleteOnClose = false)
        {
            _operations
                ...
                .Callback((string fileName, DokanFileInfo info)
                    =>
                    {
                        Trace($"{nameof(IDokanOperations.CloseFile)}[{(isDirectory ? Interlocked.Read(ref _pendingFiles) : Interlocked.Decrement(ref _pendingFiles))}] (\"{fileName}\", {info.Log()})");
                        info.Context = null;
                    })
                .Verifiable();
        }
        ...
        internal void ExpectMoveFileToFail(string path, string destinationPath, bool replace, NtStatus result)
        {
            _operations
                ...
                .Returns(result)
                .Callback((string oldName, string newName, bool _replace, DokanFileInfo info)
                    => Trace($"{nameof(IDokanOperations.MoveFile)}[{Interlocked.Add(ref _pendingFiles, 2)}] **{result}** (\"{oldName}\", \"{newName}\", {_replace}, {info.Log()})"))
                .Verifiable();
